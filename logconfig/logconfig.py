...
def initialize_logging(syslog_tag, syslog_facility, loggers,
    ...
    base_fmt = ('%(name)s:%(levelname)s %(message)s:%(pathname)s:%(funcName)s:%(lineno)s')

    tornado_fmt= \
            '%(color)s[%(levelname)1.1s %(asctime)s %(module)s:%(funcName)s:%(lineno)d]%(end_color)s %(message)s'
    ...
    cfg = {
        ...
        'formatters': {
            ...
            'tornado': {
                '()': TornadoLogFormatter,
                'format': '%s %s' %(syslog_tag,tornado_fmt),
                'color': True
            },
        },
        'handlers': {
